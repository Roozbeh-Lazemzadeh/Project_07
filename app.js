...
function createTodo(todo, index) {
	...
	}" type="text" id="${index}" value="${todo.title}" >
	</div>
	<div>
	...
	<button class="btn"><i id="submit_edit" class="fa fa-check" onClick="updateTodo(${index})"></i></button>
	</div>`;
	return todoElement;
}
...
function updateTodo(index) {
	todos[index].title = document.getElementById(index).value;
	todos[index].isEdit = !todos[index].isEdit;
	render();
}
